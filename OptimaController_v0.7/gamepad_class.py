...
class Gamepad:
    ...
    def gamepad_init(self):
        ...
        try:
            self.winmmdll = ctypes.WinDLL('winmm.dll')

            # [joyGetNumDevs](https://docs.microsoft.com/en-us/windows/win32/api/joystickapi/nf-joystickapi-joygetnumdevs)
            """
            UINT joyGetNumDevs();
            """
            self.joyGetNumDevs_proto = ctypes.WINFUNCTYPE(ctypes.c_uint)
            self.joyGetNumDevs_func = self.joyGetNumDevs_proto(("joyGetNumDevs", self.winmmdll))
            ...
            sizeof(JOYCAPS): 728
            """
            self.joyGetDevCaps_proto = ctypes.WINFUNCTYPE(ctypes.c_uint, ctypes.c_uint, ctypes.c_void_p, ctypes.c_uint)
            self.joyGetDevCaps_param = (1, "uJoyID", 0), (1, "pjc", None), (1, "cbjc", 0)
            self.joyGetDevCaps_func = self.joyGetDevCaps_proto(("joyGetDevCapsW", self.winmmdll), self.joyGetDevCaps_param)
            ...
            sizeof(JOYINFOEX): 52
            """
            self.joyGetPosEx_proto = ctypes.WINFUNCTYPE(ctypes.c_uint, ctypes.c_uint, ctypes.c_void_p)
            self.joyGetPosEx_param = (1, "uJoyID", 0), (1, "pji", None)
            self.joyGetPosEx_func = self.joyGetPosEx_proto(("joyGetPosEx", self.winmmdll), self.joyGetPosEx_param)
            print("end of try")
        except:
            print('out of try')
            self.winmmdll = None
    ...
    def joyGetDevCaps(self, uJoyID):
        try:
            ...
            ret_val = self.joyGetDevCaps_func(p1, p2, p3)
            ret = (False, None) if ret_val != self.JOYERR_NOERROR else (True, JOYCAPS(buffer))
        except:
            ret = False, None
        return ret
    ...
    def joyGetPosEx(self, uJoyID):
        try:
            buffer = (ctypes.c_uint32 * (JOYINFOEX.SIZE // 4))()
            buffer[0] = JOYINFOEX.SIZE
            buffer[1] = self.JOY_RETURNALL
            p1 = ctypes.c_uint(uJoyID)
            p2 = ctypes.cast(buffer, ctypes.c_void_p)
            ret_val = self.joyGetPosEx_func(p1, p2)
            ret = (False, None) if ret_val != self.JOYERR_NOERROR else (True, JOYINFOEX(buffer))
        except:
            ret = False, None
        return ret
